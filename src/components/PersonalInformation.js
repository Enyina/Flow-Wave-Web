      ...
      <main className="flex flex-col items-center px-4 lg:px-0 pb-32">
        <div className={`w-full max-w-lg ${hasAnimated ? 'animate-fade-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col items-center gap-6 mb-10">
            {/* Profile Image */}
          <div className="w-25 h-25 rounded-full overflow-hidden bg-neutral-light relative">
            <img
          ...
          {/* Edit Avatar Button */}
          <button onClick={handleEditAvatarClick} className="flex items-center gap-2 text-neutral-gray hover:text-neutral-dark transition-colors">
            <span className="text-xs font-normal">{uploading ? 'Uploading...' : 'Edit Avatar'}</span>
            ...
            {personalDetails.map((detail, index) => (
              <div
                ...
                className={`flex items-center justify-between p-4 bg-secondary-light rounded-lg ${
                  detail.hasArrow ? 'cursor-pointer hover:bg-secondary-light/80' : ''
                } transition-colors`}
                onClick={detail.onClick}
              >
                <span className="text-neutral-dark font-normal">
                  {detail.label}
                </span>
                <div className="flex items-center gap-2">
                  {detail.value && (
                    <span className="text-neutral-dark font-normal text-right">
                      {detail.value}
                    </span>
                  )}
                  {detail.hasArrow && (
