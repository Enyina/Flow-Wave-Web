...
const Recipients = ({ onBack, onAddRecipient, onSelectRecipient, onLogout }) => {
      ...
      <main className="flex flex-col items-center px-4 lg:px-0 pb-24">
        <div className={`w-full max-w-lg ${hasAnimated ? 'animate-fade-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
          ...
          <div className="space-y-6">
            {/* Search Box */}
            <div className={`relative ${hasAnimated ? 'animate-slide-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.4s' }}>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 bg-gray-200 dark:bg-dark-bg rounded-full flex items-center justify-center">
                  <img 
                    src="https://api.builder.io/api/v1/image/assets/TEMP/79c1f530db1139532d061067f8d65edb885258a8?width=46" 
                    alt="Add" 
                    className="w-6 h-6 rounded-full"
                  />
                </div>
                <span className="text-neutral-dark dark:text-dark-text font-medium">Add New Recipient</span>
              </div>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-neutral-dark dark:text-dark-text transform -rotate-90">
            ...
            {filteredRecipients.length === 0 && searchTerm && (
              <div className={`text-center py-8 ${hasAnimated ? 'animate-fade-in animate-once' : 'opacity-0'}`} style={{ animationDelay: '1s' }}>
                <p className="text-neutral-gray">No recipients found matching "{searchTerm}"</p>
              </div>
            )}
          </div>
        </div>
