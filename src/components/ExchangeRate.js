...
const ExchangeRate = () => {
  ...
  const { fromCurrency, toCurrency, sendAmount, receiveAmount, updateReceiveAmount } = useCurrency();
  const { transactionData, updateTransactionData } = useFlow();
  ...
  const handleTargetAmountChange = (e) => {
    ...
    if (value && parseFloat(value) > 0) {
      ...
      const calculatedReceiveAmount = (parseFloat(value) * rate).toFixed(2);
      updateReceiveAmount(calculatedReceiveAmount);
      setErrors({});
    } else {
      updateReceiveAmount('0.00');
    }
  };

  const handleContinue = () => {
    if (!targetAmount || targetAmount === '' || parseFloat(targetAmount) <= 0) {
      setErrors({ amount: 'Please enter a valid amount' });
      return;
    }

    // Update transaction data with exchange information
      ...
      <main className="flex flex-col items-center px-4 lg:px-0 pb-24">
        <div className={`w-full max-w-lg ${hasAnimated ? 'animate-fade-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
          ...
          <div className={`mb-6 ${hasAnimated ? 'animate-slide-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.4s' }}>
            <div className="p-4 rounded-lg bg-secondary-light border-2 border-transparent">
              <div className="flex justify-between items-center mb-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-lg font-bold text-neutral-gray">{sendAmount}</span>
                  </div>
                </div>
          ...
          <div className={`flex items-center justify-center mb-6 ${hasAnimated ? 'animate-fade-in animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.6s' }}>
            <div className="text-center">
              <div className="flex items-center justify-center gap-3 mb-4">
                ...
                <span className="text-lg font-bold text-primary-blue">
                  1 {fromCurrency.code} = {getExchangeRate().toFixed(4)} {toCurrency.code}
                </span>
              </div>
              <p className="text-sm text-neutral-gray">Current Exchange Rate</p>
            </div>
          </div>

          {/* Target Amount Input */}
          <div className={`mb-6 ${hasAnimated ? 'animate-slide-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.8s' }}>
            <div className={`p-4 rounded-lg bg-secondary-light border-2 ${errors.amount ? 'border-error' : 'border-transparent'}`}>
              <div className="flex justify-between items-center mb-4">
                <div className="flex-1">
                  <p className="text-neutral-dark dark:text-dark-text text-sm mb-3">Enter {toCurrency.code} Amount</p>
                  <div className="flex items-center gap-2">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    <input
                      ...
                      className="text-lg font-bold text-neutral-gray bg-transparent border-none outline-none w-full"
                      placeholder="0.00"
                    />
                  </div>
                </div>
              ...
              {errors.amount && (
                <p className="text-error text-xs text-center animate-slide-in-down animate-once">
                  {errors.amount}
                </p>
              )}
            </div>
          ...
          {/* Summary Section */}
          <div className={`mb-8 ${hasAnimated ? 'animate-fade-in animate-once' : 'opacity-0'}`} style={{ animationDelay: '1s' }}>
            <div className="p-4 rounded-lg bg-primary-light/20">
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-neutral-dark dark:text-dark-text">You Send:</span>
                  <span className="font-medium">{formatCurrency(sendAmount, fromCurrency.code)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-neutral-dark dark:text-dark-text">Receiver Gets:</span>
                  <span className="font-medium">{formatCurrency(receiveAmount, toCurrency.code)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-neutral-dark dark:text-dark-text">Fee:</span>
                  <span className="font-medium">{formatCurrency(calculateFee(), fromCurrency.code)}</span>
                </div>
                <div className="border-t pt-3 flex justify-between font-bold">
                  <span className="text-neutral-dark dark:text-dark-text">Total Pay:</span>
                  <span>{formatCurrency(calculateTotalPay(), fromCurrency.code)}</span>
                </div>
              </div>
            </div>
          </div>
