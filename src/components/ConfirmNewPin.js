...
const ConfirmNewPin = () => {
  ...
  const handleContinue = () => {
    if (confirmPin.every(digit => digit !== '')) {
      const originalPin = sessionStorage.getItem('newPin');
        ...
        <div className={`w-full max-w-lg ${hasAnimated ? 'animate-fade-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col items-center gap-10">
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              ...
              {error && (
                <p className="text-red-500 text-sm font-normal font-inter">
                  {error}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center justify-center gap-6">
              {confirmPin.map((digit, index) => (
                <input
                  ...
                  className={`w-12 h-16 rounded-lg bg-primary-light border-0 text-center text-xl font-bold text-neutral-dark focus:outline-none focus:ring-2 focus:ring-primary-blue ${error ? 'ring-2 ring-red-500' : ''}`}
                />
              ))}
            </div>
            ...
            <button
              ...
              disabled={!confirmPin.every(digit => digit !== '')}
