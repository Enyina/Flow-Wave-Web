...
const AccountPin = () => {
  ...
  const getDescription = () => {
    switch (action) {
      case 'change-email':
        return 'Enter a your 4-digit pin to change email address';
      case 'change-mobile':
        return 'Enter a your 4-digit pin to change mobile number';
      default:
        return 'Enter a your 4-digit pin to change email address';
    }
  };
    ...
    <div className="min-h-screen bg-white dark:bg-dark-bg transition-colors duration-300">
      ...
      <main className="flex flex-col items-center px-4 lg:px-0 pb-32">
        <div className={`w-full max-w-lg ${hasAnimated ? 'animate-fade-in-up animate-once' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
          ...
          {/* Error Message */}
          {error && (
            <p className="text-error text-sm text-center mb-6 animate-shake">
              {error}
            </p>
          )}
